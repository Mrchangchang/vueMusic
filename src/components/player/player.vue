<template>
  <div class="player" v-show="playList.length > 0">
        ...
        @touchmove.prevent="middleTouchMove" @touchend="middleTouchEnd">
          ...
          <Scroll class="middle-r" ref="lyricList" :data="currentLyric && currentLyric.lines">
            <div class="lyric-wrapper" v-if="currentLyric">
              <p ref="lyricLine" :class="{'current': $index == currentLineNum}"
                 class="text" v-bind:key="$index" v-for="(line, $index) in currentLyric.lines">
                {{line.txt}}
              </p>
            </div>
          </Scroll>
    ...
    <audio :src="currentSong.url + key" ref="audio" @canplay="ready" @error="error"
...
//      this.$refs.cdWrapper.addEventListener('transitionend', done)
  ...
  watch: {
    currentSong (newSong, oldSong) {
      ...
      getSongKey(this.currentSong.mid).then(res => {
        let data = res.data
        if (data.code === 0) {
          this.key = data.data.items[0].vkey
          this.$nextTick(() => {
            this.getLyric()
            this.$refs.audio.play()
          })
